...
from glm.tokenization_chatglm import ChatGLMTokenizer
import torch
import deepspeed
import argparse
...
def set_args():
    ...
    parser.add_argument('--train_batch_size', default=5, type=int, help='')
    parser.add_argument('--gradient_accumulation_steps', default=1, type=int, help='')
    parser.add_argument('--output_dir', default='output_dir_lora_glm_2task/', type=str, help='')
    parser.add_argument('--log_steps', type=int, default=10, help='')
    parser.add_argument('--max_len', type=int, default=428, help='256')
    parser.add_argument('--max_src_len', type=int, default=256, help='150')
...
def main():
    ...
    for i_epoch in range(args.num_train_epochs):
        ...
        with open(os.path.join(save_dir, "adapter_config.json"), "r", encoding="utf-8") as fh:
            adapter_config = json.load(fh)
            print(adapter_config)
            if adapter_config["inference_mode"] is True:
                adapter_config["inference_mode"] = False
        fin = open(os.path.join(save_dir, "adapter_config.json"), "w", encoding="utf-8")
        json.dump(adapter_config, fin, ensure_ascii=False, indent=4)
        fin.close()


if __name__ == "__main__":
    main()
    # CUDA_VISIBLE_DEVICES=0 deepspeed --master_port 5555 finetuning_glm.py
